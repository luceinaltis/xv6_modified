...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  push_mlfq(p, 0);  // When process is allocated, it pushed into MLFQ
  p->pstride = s_table;
  p->schedstate = MLFQ;
  return p;
}
...
exit(void)
{
  ...
  if(curproc->schedstate == STRIDE){       // When this process is in stride scheduling
    // Adjust ticket, stride
    curproc->schedstate = UNUSED;

    s_table->tickets += curproc->pstride->tickets;
    s_table->stride = (stride1/(s_table->tickets));

    // Reset stride info
    curproc->pstride->state = EMPTY;
  }
  else if(curproc->schedstate == MLFQ){    // When this process is in mlfq scheduling
    pop_mlfq(curproc);
  }
