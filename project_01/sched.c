...
pick_mlfq(void)
{
  ...
  for(int prior = 0; prior < 3; ++prior){
      ...
      {   
          ...
          if(m_table[prior].table[recent].state == USED && m_table[prior].table[recent].proc->state == RUNNABLE)
          {
              m_table[prior].recent_index = recent;
              return m_table[prior].table[recent].proc;
          }
      }
...
// Find mlfq node pointing parameter proc
// And pop from queue
int
pop_mlfq(struct proc* this)
{
    if(this->schedstate != MLFQ)
        return -1;
...
int
push_mlfq(struct proc* this, int prior)
{
    if(prior < 0 || prior > 2)
    {
        return -1;
...
int
move_prior(struct proc* this, int prior) 
{
...
//   struct mlfq_node *m;
  ...
  for(;;){
    ...
    for(cand = ptable.proc; cand < &ptable.proc[NPROC]; cand++){
        ...
        if(p < ptable.proc || &ptable.proc[NPROC] < p)
        {
            p = cand;
        }
    ...
    if(ret == s_table){
        ...
        if(p == 0){
            ...
            for(s = &s_table[1]; s < &s_table[NPROC]; ++s){
                if(s->state == EMPTY || s->proc->state != RUNNABLE)
                    continue;
...
// Priority boost evry 100 ticks
void
prior_boost(void)
{  
    struct proc* p;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; ++p){
        if(p->schedstate == STRIDE || p->schedstate == EMPTY)
            continue;
        move_prior(p, 0);
    }
